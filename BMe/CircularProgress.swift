...
//  CircularProgress
//
//  Created by parry on 1/14/15.
//  Credit KDCircularProgress
// https://github.com/kaandedeoglu/KDCircularProgress
//  Copyright (c) 2017 parry. All rights reserved.
//
// Easy to use animating Circular Progress View
// can be used programmatically or as IBDesignable View
// to use  1.set an angle 2.call animate function to angle, can reset back to zero if necessary 3.view the customDisplay settings to change UI

import UIKit

struct customDisplaySettings {
    static let trackColor = UIColor.blue
    static let glowAmount: CGFloat = 0.5
    static let progressThickness: CGFloat = 0.1
    static let trackThickness: CGFloat = 0.3
    static let progressColorArray = [ UIColor.yellow, UIColor.green, UIColor.purple, UIColor.orange, UIColor.red]
    static let angle = Double(90)
...
public class CircularProgress: UIView, CAAnimationDelegate {
    ...
    private func refreshValues() {
        progressLayer.angle = customDisplaySettings.angle
        progressLayer.startAngle = customDisplaySettings.startAngle
        progressLayer.clockwise = clockwise
        progressLayer.roundedCorners = roundedCorners
        progressLayer.lerpColorMode = lerpColorMode
        progressLayer.gradientRotateSpeed = gradientRotateSpeed
        progressLayer.glowAmount = customDisplaySettings.glowAmount
        progressLayer.glowMode = glowMode
        progressLayer.progressThickness = customDisplaySettings.progressThickness
        progressLayer.trackColor = customDisplaySettings.trackColor
        progressLayer.trackThickness = customDisplaySettings.trackThickness
        progressLayer.colorsArray = customDisplaySettings.progressColorArray
    ...
    /**
     Animates circular view to a particular angle
     
     This method takes in a double value which is the angle of the progress.  Note - The angle will go back if passed a lesser angle than the current.  Animates speciifcally to the angle passed in regardless of where it is.
     
     :param: angle the angle constant that the circle should change to
